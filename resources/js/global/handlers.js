        ...
        Vue.prototype.$handlers = {
            ...
            create_item: (entity, data) => {
                ...
                if(['coins', 'types'].includes(entity)) {
                    ...
                    item.comment_public     =   d?.comment                  ? d.comment : null
            ...
            show_item_data: (language, entity, item_data, key, section) => {
                ...
                if(['coins', 'types'].includes(entity)) {
                    ...
                    else if (key === 'authority')           { return to_string.basic(d?.authority?.kind, language) }
                    ...
                    else if (key === 'period')  	        { return to_string.basic(d?.date?.period, language) }

                    // Basics
                    else if (['material', 'denomination', 'standard'].includes(key)) { return to_string.basic(d?.[key], language) }
                    ...
                    // Hoard and Findsport
                    else if (['hoards', 'findspots'].includes(key)) { return !d?.[key] ? '--' : d[key].map((data) => { return to_string.hoard_findspot(data, language) }).join('\n') }
                    else if (['hoard', 'findspot'].includes(key)) { return to_string.hoard_findspot(d?.[key], language) }

                    // About
                    else if (key === 'comment_public')      { return to_string.simple_text(d?.comment) }
            ...
            format: {
                ...
                cn_public_link: (item)                      => {
                    if (item?.id && (item?.public === 1 || item?.dbi?.public === 1)) {
                        return '<a href="' + urls.cn + (item.kind?.toLowerCase() === 'coin' ? 'coins?id=' : 'types/') + item.id + '" target="_blank">&nbsp;&ast;</a>'
                    }
                    else {
                        return ''
                    }
...
const to_string = {
    ...
    mint: (data, language) => {
        return !data?.id ? '--' : 
        (format_id(data.id) + data.text[language] + 
    ...
    basic: (data, language) => { // works for authority, period, material, denomination, standard
        return !data?.id ? '--' : (format_id(data.id) + data.text[language] + make_element.resource_link(data.link))        
    },
    
    design_legend: (key, data, language) => {
        return !data?.id ? '--' :
        (format_id(data.id) + (key === 'design' ? data.text[language] : ('<span class="font-weight-thin">' + data.string + '</span>')))
    },
    monogram_symbol: (key, data, language) => {
        return !data?.id ? '--' : '<div class="d-flex component-wrap align-start pr-2">' +
            ...
            '<div>' + 
            format_id(data.id) + data.position.text[language] + ': ' +
...
const format_id = (id) => { return !id ? '' : ('(&nbsp;' + id + '&nbsp;)&ensp;') }
